...
### A. Veri Seti (Simülasyon İçin: RetailRocket)
Yerel veritabanımızın sıfırıncı gününde (Day-0) hiç tıklama ve kullanıcı verisi olmadığı için sistemi test edemezdik. Sistemi %96 isabet oranında çalıştırdığını kanıtlamak adına Kaggle'dan devasa bir açık kaynaklı veri seti (`retail_rocket`) indirilmiş ve model bu verilerle önceden eğitilmiştir (Pre-training).
